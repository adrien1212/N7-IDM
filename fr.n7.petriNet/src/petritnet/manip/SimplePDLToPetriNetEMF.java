 		...
 		process.getProcessElements().stream().forEach(pe -> {
 			...
 			// WorkSequence
 				...
 				Arc aws = myFactory.createArc();
 	 			...
 	 			aws.setSource((Noeud) petrinet.getPetriElements().stream().filter(p -> p instanceof Noeud).filter(n -> ((Noeud) n).getName()
 	 			...
 	 			aws.setTarget((Noeud) petrinet.getPetriElements().stream().filter(p -> p instanceof Noeud).filter(n -> ((Noeud) n).getName()
 	 		...
 	 		// Resources
 				...
 				for (Need ne : rs.getNeeds()) {
 					Arc anLoad = myFactory.createArc();
 	 				...
 	 				anLoad.setTarget((Noeud) petrinet.getPetriElements().stream().filter(p -> p instanceof Noeud).filter(n -> ((Noeud) n).getName()
 	 				...
 	 				anRelease.setSource((Noeud) petrinet.getPetriElements().stream().filter(p -> p instanceof Noeud).filter(n -> ((Noeud) n).getName()
